...
func TestBitConvert(t *testing.T) {
	tests := []struct {
		...
		{MustCreateBitType(64), float32(893.22356), func() uint64 {
			val := uint64(math.Trunc(893.22356))
		...
		{MustCreateBitType(21), "32", func() uint64 {
			val, err := strconv.ParseUint("32", 10, 64)
			if err != nil {
				return 0
			}
			return val
